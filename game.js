...
// 게임 종료 및 재시작을 묻는 함수
// 게임 종료 및 재시작을 묻는 함수
async function handleGameOver(stage) {
  while (true) {
    ...
    if (restartChoice === '1') {
      ...
      stage.value = 1; // 스테이지 리셋 및 새 게임 시작
      return true; // 게임 재시작
...
export async function startGame() {
  ...
  let stage = { value: 1 }; // stage를 객체로 만들어 참조형으로 사용
  ...
  while (restart) {
    ...
    while (stage.value <= 10) {
      console.log(chalk.cyan(`현재 스테이지: ${stage.value}`));

      const monster = new Monster(stage.value);
      console.log(chalk.yellowBright(`몬스터 생성됨: ${monster.name}`));

      // battle 함수 호출 후 결과에 따라 다음 단계 결정
      const battleResult = await battle(stage.value, player, monster);

      if (battleResult === 'won') {
        console.log(chalk.gray(`\n=======================================`));
        console.log(chalk.yellowBright(`\n${stage.value} 스테이지 클리어!!`));
        ...
        console.clear();
        stage.value++; // 다음 스테이지 진행
      } else if (battleResult === 'lost') {
        restart = await handleGameOver(stage);
        if (!restart) return start(); // 메인 화면으로 이동
        break; // 내부 while 루프 종료
      } else if (battleResult === 'escaped') {
        console.log(
          chalk.yellowBright(`\n${stage.value} 스테이지를 도망친 후 잠시 숨을 고릅니다...`),
        );

        const restResult = await restingStage(player, stage.value); // 휴식 스테이지 이동
